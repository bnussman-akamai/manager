...
interface Options {
  ...
  routePath?: string;
  routerOptions?: TanStackRouterOptions;
  theme?: 'dark' | 'light';
}
...
export const wrapWithTheme = (ui: any, options: Options = {}) => {
  ...
  const indexRoute = createRoute({
    ...
    path: options.routerOptions?.initialRoute ?? '/',
  });

  const tree = options.routerOptions?.useFullRouter
    ? migrationRouteTree
  ...
  const router: AnyRouter = createRouter({
    history: createMemoryHistory({
      initialEntries: options.MemoryRouter?.initialEntries
        ? (options.MemoryRouter.initialEntries as string[])
        : [options.routerOptions?.initialRoute ?? '/'],
    }),
    routeTree: tree,
    defaultPendingMs: 0,
