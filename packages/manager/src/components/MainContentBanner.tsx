...
export const MainContentBanner = React.memo(() => {
  ...
  const { data: mainContentBannerPref } = usePreferences(
    (preferences) => preferences?.main_content_banner_dismissal
  );
  const { mutateAsync: updatePreferences } = useMutatePreferences();

  const handleDismiss = (key: string) => {
    const existingMainContentBannerDismissal = mainContentBannerPref ?? {};
  ...
  const hasDismissedBanner =
    ...
    mainContentBannerPref?.[flags.mainContentBanner.key];
