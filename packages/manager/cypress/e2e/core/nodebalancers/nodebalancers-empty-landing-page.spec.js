...
describe('NodeBalancers empty landing page', function () {
    /**
     ...
     */
    it('shows the empty state when there are no nodebalancers', function () {
        (0, nodebalancers_1.mockGetNodeBalancers)([]).as('getNodebalancers');
        ...
        cy.findByText('Add high availability and horizontal scaling to web applications hosted on Linode Compute Instances.').should('be.visible');
        ...
        cy.findByText('Configuration Options for NodeBalancers').should('be.visible');
        cy.findByText('View additional NodeBalancer documentation').should('be.visible');
        // checks that videos are visible
        cy.findByText('Video Playlist').should('be.visible');
        cy.findByText('Getting Started With NodeBalancers | How To Prepare For High Server Traffic').should('be.visible');
        cy.findByText('Linode NodeBalancers Explained | Manage Scale with Transparent Load Distribution').should('be.visible');
        cy.findByText('Load Balancing on an LKE Kubernetes Cluster').should('be.visible');
