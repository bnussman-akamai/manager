...
describe('Domains empty landing page', function () {
    /**
     ...
     */
    it('shows the empty state when there are no domains', function () {
        (0, domains_1.mockGetDomains)([]).as('getDomains');
        cy.visitWithLogin('/domains');
        cy.wait(['@getDomains']);
        // confirms helper text
        cy.findByText('Easy domain management').should('be.visible');
        cy.findByText('A comprehensive, reliable, and fast DNS service that provides easy domain management for no additional cost.').should('be.visible');
        ...
        cy.findByText('Linode DNS Manager | Total Control Over Your DNS Records').should('be.visible');
        cy.findByText('Using Domains with Your Server | Common DNS Configurations').should('be.visible');
